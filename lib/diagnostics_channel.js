...
class StoreBinding {
  ...
  onEnter(data) {
    ...
    this.stack.push(resource[this.store.kResourceStore]);

    resource[this.store.kResourceStore] = this.build(data);
  }

  onExit() {
    if (!this.stack.length) return;
    const resource = executionAsyncResource();
    resource[this.store.kResourceStore] = this.stack.pop();
  }
}
